...
/// Read a process's current working directory by pid. macOS implementation uses
...
fn pid_cwd(pid: i32) -> Option<String> {
    ...
    // proc_pidinfo returns bytes written; a partial/zero/negative result means
    // the process is gone, a zombie, or inaccessible.
    if n <= 0 || n != size {
        return None;
    }
    // vip_path is `[[c_char; 32]; 32]` (1024 bytes == MAXPATHLEN) — a flat
    // NUL-terminated C string split into a 2D array. Reinterpret as flat bytes.
    let raw = &info.pvi_cdir.vip_path;
    debug_assert_eq!(std::mem::size_of_val(raw), 1024);
    let flat: &[libc::c_char; 1024] = unsafe { &*(raw.as_ptr() as *const [libc::c_char; 1024]) };
    let bytes: Vec<u8> = flat.iter().take_while(|&&c| c != 0).map(|&c| c as u8).collect();
    if bytes.is_empty() {
        return None;
    }
    std::ffi::OsString::from_vec(bytes).into_string().ok()
}

#[cfg(not(target_os = "macos"))]
fn pid_cwd(_pid: i32) -> Option<String> {
    None
}

/// Direct child pids of `ppid` (macOS).
#[cfg(target_os = "macos")]
fn child_pids(ppid: i32) -> Vec<i32> {
    // First call sizes the buffer; proc_listchildpids returns bytes written.
    let needed = unsafe { libc::proc_listchildpids(ppid, std::ptr::null_mut(), 0) };
    if needed <= 0 {
        return Vec::new();
    }
    let cap = (needed as usize) / std::mem::size_of::<i32>() + 8;
...
/// Collect the cwds of `root` and all its descendants (macOS). Bounded by a
/// node budget and a visited set so a pathological tree can't run away.
#[cfg(target_os = "macos")]
fn collect_tree_cwds(root: i32) -> Vec<String> {
...
#[cfg(not(target_os = "macos"))]
fn collect_tree_cwds(_root: i32) -> Vec<String> {
    Vec::new()
}
...
impl PtySession {
    ...
    /// Working directory that reflects this terminal's worktree activity.
    ///
    /// Walks the foreground process subtree and, if any descendant sits inside
    /// one of `worktree_paths`, returns that cwd (deepest worktree wins). This
    /// is what catches a root-launched agent doing work via short-lived
    /// `cd <worktree> && …` subshells — the agent's own process stays pinned at
    /// the repo root, but its descendants briefly run in the worktree. When no
    /// descendant is in a worktree, returns the root process cwd (the main repo,
    /// or a `claude -w` session whose process already lives in the worktree).
    /// Returns None if the process is gone or the platform is unsupported.
    pub fn foreground_cwd(&self, worktree_paths: &[String]) -> Option<String> {
        // Root the walk at the PTY's direct child (top of the tree) so the whole
        // subtree is covered; fall back to the foreground process-group leader.
        // try_lock so we never block on the reader thread holding the child mutex.
        let root = self
            .child
            .try_lock()
            .ok()
            ...
            .map(|p| p as i32)
            .or_else(|| self.master.process_group_leader())?;

        let cwds = collect_tree_cwds(root);
        let mut best: Option<(usize, String)> = None;
        for cwd in &cwds {
            for wt in worktree_paths {
                if path_within(wt, cwd) && best.as_ref().map_or(true, |(len, _)| wt.len() > *len) {
                    best = Some((wt.len(), cwd.clone()));
                }
            }
        }
